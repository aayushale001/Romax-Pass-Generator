...
export const brandLockSchema = z.object({
  businessName: z.literal(true),
  logo: z.literal(true),
  primaryColor: z.literal(true),
  tone: z.literal(true),
  templateFamily: z.boolean(),
});
...
const visualStyleSchema = z.object({
  ...
  texture: z.enum(["none", "fine-lines", "dot-grid", "topographic"]),
  decorativeMotif: z.enum([
    "none",
    "abstract-waves",
    "geometric-orbit",
    "manga-burst",
...
const artDirectionSchema = z.object({
  badgeText: z.string().min(1).max(18),
  tagline: z.string().min(1).max(56),
  artPrompt: z.string().max(220).nullable(),
});

const cardBackgroundSchema = z.object({
  type: z.enum(["solid", "gradient", "image", "image-overlay", "pattern"]),
  color: hexColorSchema.nullable().optional(),
  imageUrl: z.string().nullable().optional(),
  imageRole: z
    .enum(["brandBackground", "memberPhoto", "decorativeArt", "heroImage"])
    .nullable()
    .optional(),
  overlayColor: hexColorSchema.nullable().optional(),
  overlayOpacity: z.number().min(0).max(0.92).nullable().optional(),
  pattern: z
    .enum(["none", "grid", "diagonal", "dots", "waves", "noise"])
    .nullable()
    .optional(),
});

const structuredCardBackgroundSchema = z.object({
  type: z.enum(["solid", "gradient", "image", "image-overlay", "pattern"]),
  ...
  imageRole: z
    .enum(["brandBackground", "memberPhoto", "decorativeArt", "heroImage"])
    .nullable(),
  overlayColor: hexColorSchema.nullable(),
  overlayOpacity: z.number().min(0).max(0.92).nullable(),
  pattern: z.enum(["none", "grid", "diagonal", "dots", "waves", "noise"]).nullable(),
});

const cardBlueprintSchema = z.object({
  structure: z.enum([
    "official-grid",
    "split-panel",
    "poster",
  ...
  brandZone: z.enum(["top-left", "top-center", "side-panel", "bottom-left"]),
  identityZone: z.enum(["center", "left", "right", "bottom-left"]),
  mediaZone: z.enum([
    "none",
    "left-panel",
    "right-panel",
  ...
  fieldZone: z.enum(["grid", "stack", "compact-row", "badges"]),
});

const templateFamilySchema = z.enum([
  "institutional",
  "identity",
  "event-access",
  "premium",
...
const baseCardConceptShape = {
  id: z.string().min(1),
  conceptName: z.string().min(1).max(60),
  shortReason: z.string().min(1).max(240),
  ...
  layout: z.object({
    logoPosition: z.enum(["top-left", "top-center"]),
    photoShape: z.enum(["circle", "rounded-square", "square"]),
    qrPosition: z.enum(["bottom-right", "bottom-center", "back"]),
    fieldDensity: z.enum(["low", "medium", "high"]),
  }),
  typography: z.object({
    style: z.enum(["formal", "modern", "bold", "clean", "expressive"]),
    headingWeight: z.enum(["medium", "semibold", "bold", "black"]),
  }),
  recommendedFields: z.array(memberFieldKeySchema).min(3).max(13),
  brandLock: brandLockSchema,
};

export const cardConceptSchema = z.object({
  ...baseCardConceptShape,
  colors: z.object({
    background: hexColorSchema,
    text: hexColorSchema,
    accent: hexColorSchema,
    secondaryBackground: hexColorSchema.nullable().optional(),
  }),
  visualStyle: visualStyleSchema.optional(),
  artDirection: artDirectionSchema.optional(),
});

const structuredOutputCardConceptSchema = z.object({
  ...baseCardConceptShape,
  background: structuredCardBackgroundSchema,
  colors: z.object({
    background: hexColorSchema,
    text: hexColorSchema,
    accent: hexColorSchema,
    secondaryBackground: hexColorSchema.nullable(),
  }),
  visualStyle: visualStyleSchema,
  artDirection: artDirectionSchema,
});

export const legacyConceptsResponseSchema = z.object({
  concepts: z.array(structuredOutputCardConceptSchema).length(4),
});

export const refinementPatchPathSchema = z.enum([
  "brandProfile.businessName",
  "brandProfile.logoUrl",
  "brandProfile.logoMode",
  "brandProfile.primaryColor",
  "brandProfile.secondaryColor",
  "brandProfile.brandColorUsage",
  "brandProfile.selectedHeroImageUrl",
  "brandProfile.selectedBackgroundImageUrl",
  "brandProfile.selectedProfileImageUrl",
  "cardConcept.conceptName",
  "cardConcept.shortReason",
  "cardConcept.templateFamily",
  "cardConcept.templateId",
  "cardConcept.visualTone",
  "cardConcept.brandColorUsage",
  "cardConcept.colors.background",
  "cardConcept.colors.text",
  "cardConcept.colors.accent",
  "cardConcept.colors.secondaryBackground",
  "cardConcept.background.type",
  "cardConcept.background.color",
  "cardConcept.background.imageUrl",
  "cardConcept.background.imageRole",
  "cardConcept.background.overlayColor",
  "cardConcept.background.overlayOpacity",
  "cardConcept.background.pattern",
  "cardConcept.blueprint.structure",
  "cardConcept.blueprint.orientation",
  "cardConcept.blueprint.brandZone",
  "cardConcept.blueprint.identityZone",
  "cardConcept.blueprint.mediaZone",
  "cardConcept.blueprint.qrZone",
  "cardConcept.blueprint.fieldZone",
  "cardConcept.layout.logoPosition",
  "cardConcept.layout.photoShape",
  "cardConcept.layout.qrPosition",
  "cardConcept.layout.fieldDensity",
  "cardConcept.typography.style",
  "cardConcept.typography.headingWeight",
  "cardConcept.visualStyle.backgroundTreatment",
  "cardConcept.visualStyle.accentShape",
  "cardConcept.visualStyle.texture",
  "cardConcept.visualStyle.decorativeMotif",
  "cardConcept.artDirection.badgeText",
  "cardConcept.artDirection.tagline",
  "cardConcept.artDirection.artPrompt",
  "cardConcept.recommendedFields",
]);

export const refinementPatchSchema = z.object({
  target: z.enum(["brandProfile", "cardConcept"]),
  op: z.enum(["set", "appendUnique", "remove"]),
  path: refinementPatchPathSchema,
  value: z.object({
    stringValue: z.string().nullable(),
    numberValue: z.number().nullable(),
    booleanValue: z.boolean().nullable(),
    stringArrayValue: z.array(z.string()).nullable(),
  }),
  reason: z.string().min(1).max(180),
});

export const legacyRefineConceptResponseSchema = z.object({
  patches: z.array(refinementPatchSchema).min(1).max(12),
