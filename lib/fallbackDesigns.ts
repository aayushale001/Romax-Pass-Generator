...
export function locallyRefineGeneratedDesign(
  ...
  const asksForPhoto = /photo|portrait|picture|headshot|selfie/.test(lower);
  ...
  const mood: GeneratedCardDesign["mood"] = asksForArt
    ? "creative"
    : asksForPhoto
      ? "identity"
      : asksForMinimal
  ...
  const requiredFields = Array.from(
    new Set<MemberFieldKey>([
      ...concept.requiredFields,
      ...(asksForArt ? (["decorativeArt"] as MemberFieldKey[]) : []),
  ...
  const refined: GeneratedCardDesign = {
    ...concept,
    name: asksForArt ? "Custom Art Layout" : `${concept.name} Refined`,
    ...
    designTokens: {
      ...
      textColor:
        asksForArt || asksForStrongColor ? text : concept.designTokens.textColor,
      backgroundMode: asksForArt
        ? "image-overlay"
        : concept.designTokens.backgroundMode,
      colorUsage:
        asksForArt || asksForStrongColor
          ? "full"
          : asksForMinimal
            ? "subtle"
            : concept.designTokens.colorUsage,
      brandAssetSource: asksForArt
        ? concept.designTokens.brandAssetSource ?? "background"
        : concept.designTokens.brandAssetSource ?? "none",
      brandAssetTreatment: asksForArt
        ? "hero-backdrop"
        : asksForStrongColor
          ? "background-emblem"
      ...
      usesPhoto: concept.designTokens.usesPhoto || asksForPhoto,
      usesDecorativeArt: concept.designTokens.usesDecorativeArt || asksForArt,
      usesLogo: brand.logoMode !== "none",
      usesQr: true,
    },
  };
